...
function sortearEquipes(){
  ...
  alert("🔴 TIME ALPHA:\n"+lista.slice(0,metade).map(p=>"- "+p.name).join("\n")+
        "\n\n🔵 TIME BRAVO:\n"+lista.slice(metade).map(p=>"- "+p.name).join("\n"));
}
...
function reprovar(id){ 
  if(confirm("Remover solicitação?")){ 
    espera=espera.filter(p=>p.id!==id); renderAdmin(); 
  } 
}

function addScore(nome){ 
  const k=parseInt(prompt(`Quantas kills ${nome} fez?`,"0"))||0; 
  const idx=aprovados.findIndex(p=>p.name===nome); 
  aprovados[idx].matches+=1; aprovados[idx].kills+=k; renderRank(); 
}
...
function editarNome(velho){ 
  ...
  if(novo){ 
    ...
    aprovados[idx].name=novo; renderRank(); 
  } 
}

function removerDoRank(nome){ 
  if(confirm("Expulsar "+nome+"?")){ 
    aprovados=aprovados.filter(p=>p.name!==nome); renderRank(); 
  } 
}
...
function uploadFoto(input){ 
  if(input.files && input.files[0]){ 
    ...
    reader.onload=e=>{bannerSalvo=e.target.result; document.getElementById('bannerImg').src=bannerSalvo;} ; 
